...
def main():
    ...
    # REMOVE THIS
#    print('insider_df data')
#    print(insider_df['SYMBOL'])
#    print(insider_df['VALUE OF SECURITY (ACQUIRED/DISPLOSED)'])
    ...
    working_df = insider_df[insider_df['CATEGORY OF PERSON'].isin(['Promoters', 'Promoter Group','Employees/Designated Employees','Other','Director','Key Managerial Personnel','Immediate relative'])]
    working_df = working_df[working_df['ACQUISITION/DISPOSAL TRANSACTION TYPE'].isin(['Buy'])]
#   working_df = working_df[working_df['MODE OF ACQUISITION'].isin(['Market Purchase', 'ESOP','Off Market','Preferential Offer','Gift','Inter-se-Transfer','Conversion of security','Others','Public Right','Bonus','Scheme of Amalgamation/Merger/Demerger/Arrangement','Market Sale'])]
...
# REMOVE THIS
#    print('working df data')
#    print (working_df['SYMBOL'])
#    print (working_df['VALUE OF SECURITY (ACQUIRED/DISPLOSED)'])
...
# UNCOMMENT THIS
    ...
    working_df = pandas.merge(left=working_df, right=bhav_df, left_on='SYMBOL', right_on='SYMBOL', how='left')
    # Left merge so that if any symbols are not found in the bhav file they are just N/A instead of dropped

    # Gather buying and selling quantities and averages
    working_df['BUYING QTY OF ALL'] = working_df.apply(lambda row: get_value('purchase_qty', row['SYMBOL'], insider_df), axis=1)
    working_df['BUYING AVG PRICE OF ALL'] = working_df.apply(lambda row: get_value('purchase_avg', row['SYMBOL'], insider_df), axis=1)
    working_df['SELLING QTY PRICE OF ALL'] = working_df.apply(lambda row : get_value('selling_qty', row['SYMBOL'], insider_df), axis=1)
    working_df['SELLING AVG PRICE OF ALL'] = working_df.apply(lambda row: get_value('selling_avg', row['SYMBOL'], insider_df), axis=1)

    working_df['BUYING QTY OF PROMOTERS/PROMOTER GROUP'] = working_df.apply(lambda row: get_value1('purchase_qty', row['SYMBOL'], insider_df), axis=1)
    working_df['BUYING AVG PRICE OF PROMOTERS/PROMOTER GROUP'] = working_df.apply(lambda row: get_value1('purchase_avg', row['SYMBOL'], insider_df), axis=1)
...
def set_signal_column_value(row):
    ...
    selling_qty = row['SELLING QTY PRICE OF ALL']
    ...
    buying_avg = row['BUYING AVG PRICE OF ALL']
...
# Called by lambda
def get_value1(mode, symbol, insider_df):
...
#    print (symbol_df['VALUE OF SECURITY (ACQUIRED/DISPLOSED)'])

    # Get the appropriate dataframe for the mode we're in
    # if mode in ['selling_qty', 'selling_avg']:
    #   symbol_df = symbol_df[symbol_df['MODE OF ACQUISITION'].isin(['Market Sale', 'ESOP','Off Market','Gift','Inter-se-Transfer','Conversion of security','Others','Scheme of Amalgamation/Merger/Demerger/Arrangement'])]
    # elif mode in ['purchase_qty', 'purchase_avg']:
    #   symbol_df = symbol_df[symbol_df['MODE OF ACQUISITION'].isin(['Market Purchase', 'ESOP','Off Market','Preferential Offer','Gift','Inter-se-Transfer','Conversion of security','Others','Public Right','Bonus','Scheme of Amalgamation/Merger/Demerger/Arrangement'])]

    #if mode in ['selling_qty', 'selling_avg']:
    #    symbol_df = symbol_df[symbol_df['ACQUISITION/DISPOSAL TRANSACTION TYPE'].isin(['Sell'])]
    #elif
    if mode in ['purchase_qty', 'purchase_avg']:
        symbol_df = symbol_df[symbol_df['ACQUISITION/DISPOSAL TRANSACTION TYPE'].isin(['Buy'])]
...
#    print (symbol_df['VALUE OF SECURITY (ACQUIRED/DISPLOSED)'])
    ...
    # Filter only for promoters and promoter group
    symbol_df = symbol_df[symbol_df['CATEGORY OF PERSON'].isin(['Promoters', 'Promoter Group','Employees/Designated Employees','Other','Director','Key Managerial Personnel','Immediate relative'])]
